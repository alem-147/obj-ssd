import torch
from torch.nn import Conv2d, Sequential, ModuleList, ReLU, BatchNorm2d, MaxPool2d
...
def create_mobilenetv1_rssd_lite(num_classes, is_test=False):
    ...
    extras = ModuleList([
        ...
        Sequential(
            Conv2d(in_channels=512+512, out_channels=128, kernel_size=1),
            ReLU(),
            SeperableConv2d(in_channels=128, out_channels=256, kernel_size=3, stride=2, padding=1),
        ),
        Sequential(
            Conv2d(in_channels=256+256, out_channels=128, kernel_size=1),
            ReLU(),
            SeperableConv2d(in_channels=128, out_channels=256, kernel_size=3, stride=2, padding=1),
        ),
        Sequential(
    ...
    pool_extras = ModuleList([
        ...
        Sequential(
            Conv2d(in_channels=512+512, out_channels=256, kernel_size=1),
            BatchNorm2d(256),
            MaxPool2d(kernel_size=3, stride=2, padding=1)
        ),
        Sequential(
            Conv2d(in_channels=256+256, out_channels=256, kernel_size=1),
            BatchNorm2d(256),
            MaxPool2d(kernel_size=3, stride=2, padding=1)
        ),
        Sequential(
    ...
    return RSSD(num_classes, base_net, source_layer_indexes,
                extras, pool_extras, classification_headers, regression_headers, is_test=is_test, config=config)


def create_mobilenetv1_rssd_lite_predictor(net, candidate_size=200, nms_method=None, sigma=0.5, device=None):
